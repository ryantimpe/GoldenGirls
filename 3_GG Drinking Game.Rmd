...
blanche <- gg_trans_1gram %>% 
  filter(word %in%  c("slut")) %>% 
  count(Season)

sophia <- gg_trans_2gram %>% 
  filter(n_gram %in% c("picture it")) %>% 
  count(Season)

dorothy <- gg_trans_2gram %>% 
...
I start by converting the episodes in each season to a running tally of time. Each half hour episode is approximately 22-minutes long, though 11 of the episodes were hour-long (44 minutes).
